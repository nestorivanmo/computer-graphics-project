...
// Keyframes
float posX = PosIni.x, posY = PosIni.y, posZ = PosIni.z, rotRodIzq = 0, rotRodDer, rotBraIzq, rotBraDer;
...
void DoMovement()
{
	...
	// Rotación de pierna izquierda
	if (keys[GLFW_KEY_2]) {
		if (rotRodIzq < 80.0f) rotRodIzq += 1.0f;
	}
	if (keys[GLFW_KEY_3]) {
		if (rotRodIzq > -45) rotRodIzq -= 1.0f;
	}
	// Rotación de pierna derecha
	if (keys[GLFW_KEY_4]) {
		if (rotRodDer < 80.0f) rotRodDer += 1.0f;
	}
	if (keys[GLFW_KEY_5]) {
		if (rotRodDer > -45) rotRodDer -= 1.0f;
	}
	// Rotación de brazo izquierdo
	if (keys[GLFW_KEY_6]) {
		if (rotBraIzq < 15.0f) rotBraIzq += 1.0f;
	}
	if (keys[GLFW_KEY_7]) {
		if (rotBraIzq > -55.0f) rotBraIzq -= 1.0f;
	}
	// Rotación de brazo derecho
	if (keys[GLFW_KEY_8]) {
		if (rotBraDer < 55.0f) rotBraDer += 1.0f;
	}
	if (keys[GLFW_KEY_9]) {
		if (rotBraDer > -15.0f) rotBraDer -= 1.0f;
	}
